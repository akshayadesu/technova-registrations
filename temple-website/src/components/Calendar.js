...
const Calendar = () => {
  ...
  const renderEventsForDate = (date) => {
    ...
    if (dayEvents.length === 0) {
      ...
      return (
        <>
          <div className="event-marker" onClick={() => setOpen(true)}>
            {dayEvents.length} events
          </div>
          <Dialog.Root open={open} onOpenChange={setOpen}>
            <Dialog.Overlay className="dialog-overlay" />
            <Dialog.Content className="dialog-content">
              <Dialog.Title className="dialog-title">Events for {date.toDateString()}</Dialog.Title>
              <Dialog.Close className="dialog-close" />
              {dayEvents.map(event => (
                <Card key={event.id} className="event">
                  <div className="event-name">{event.event_name}</div>
  ...
  const renderCalendar = () => {
    ...
    // Find the first day of the week for the startOfMonth
    const startDayOfWeek = startOfMonth.getDay();
    const currentMonth = currentDate.getMonth();

    // Create date objects for the previous month's last few days to fill the first week
    ...
    // Add dates for the next month's beginning days to fill the last week
    ...
    return (
      <div className="calendar-grid">
        {weeks.map((week, index) => (
          <div key={index} className="calendar-week">
            {week.map((date, index) => (
              <div key={index} className={`calendar-date ${date.getMonth() !== currentMonth ? 'other-month' : ''}`}>
                <div className="date">{date.getDate()}</div>
                {renderEventsForDate(date)}
              </div>
            ))}
